...
$missingTeacherid = '<p><stong>Please enter unique id!</strong></p>';
$errors="";
...
if(empty($_POST["loginid"])){
    $errors .= $missingTeacherid;   
}else{
    $uid = filter_var($_POST["loginid"], FILTER_SANITIZE_STRING);
}
...
$sql = "SELECT * FROM teacher WHERE uid='$uid' AND email='$email' AND password='$password'";

$result = mysqli_query($link, $sql);
if(!$result){
    echo '<div class="alert alert-danger">Error running the query!</div>'.$result;
    exit;
}
        //If email & password don't match print error
$count = mysqli_num_rows($result);
if($count !== 1){
    echo '<div class="alert alert-danger">Wrong Email id or Password or Unique id</div>';
}
else {
